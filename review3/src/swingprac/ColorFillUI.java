...
public class ColorFillUI {
    ...
    private JButton undoBtn; // Added for Undo
    ...
    private void optionsPanelCreate() {
        ...
        undoBtn = new JButton("Undo"); // Added for Undo
        optionsPanel.add(newGameBtn);
        optionsPanel.add(resetBtn);
        optionsPanel.add(undoBtn); // Added for Undo
        ...
        for (int i = 0; i < cells.length; i++) {
            for (int j = 0; j < cells.length; j++) {
                ...
                if (cell.owner == Owner.HUMAN) {
                    ...
                    buttons[i][j].setText(""); // Added for Undo restoration
                }
            }
        }
    }
    ...
    public JButton getUndoBtn() { // Added for Undo
        return undoBtn;
    }
